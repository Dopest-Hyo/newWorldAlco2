...
export const deleteReply = createAsyncThunk("DELETE_REPLY", async(replyId)=>{
  const response = await axios.delete(`http://localhost:3002/member/${replyId}`, );
  return replyId;
})
...
export const updateReply = createAsyncThunk(
  ...
  async ({ replyId, username, comment}) =>{
     const response = await axios.put(`http://localhost:3002/member/${replyId}`,{
    ...
    [updateReply.fulfilled]: (state, {payload})=>{
      return state.map((reply)=>{
        if(reply.id === payload.replyId){
          return{...reply, username:payload.username, comment:payload.comment};
        }else{
          return reply;
        }
